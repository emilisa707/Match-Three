    ...
    void CreateGrid()
    {
        for(int x = 0; x < gridSizeX; x++)
        {
            for(int y = 0; y < gridSizeY; y++)
            {
                ...
                //GameObject candy = ObjectPooler.GetInstanceID().SpawnFromPool(index.ToString(), pos, Quaternion.identity);
                candy.transform.parent = ObjectPooler.transform;
    ...
    private void RefillBoard()
    {
        for(int i = 0; i < gridSizeX; i++)
        {
            for(int j = 0; j < gridSizeY; j++)
            {
                if(tiles[i,j] == null)
                {
                    Vector2 tempPosition = new Vector3(startPos.x + (i * offset.x), startPos.y + (j * offset.y));
                    int index = Random.Range(0, candies.Length);
                    GameObject tile = Instantiate(candies[index], tempPosition, Quaternion.identity);
                    tile.transform.parent = ObjectPooler.transform;
                    tile.name = "(" + i + "," + j + ")";
                    tiles[i, j] = tile;
                }
            }
